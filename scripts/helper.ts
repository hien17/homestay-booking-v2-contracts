...
export class Deployer {
  ...
  async deployContract(
    ...
    log.info(
      `======================Deploy with ${
        accounts[this.accountnum].address
      } on ${hre.network.name}======================`
    );
    ...
    log.info(`1`);
    const contract = await ContractDeployer.connect(
      accounts[this.accountnum]
    ).deploy(...args);
    log.info(`2`);
    ...
    log.info(
      `Contract ${contractName} deployed at: ${contract.address}.\n- Gas Price: ${gasPrice}\n- Gas Used: ${gasUsed}\n- Transaction Fee: ${transactionFee}\n- Transaction hash: ${receipt.transactionHash}`
    );
    return deployed;
  }
